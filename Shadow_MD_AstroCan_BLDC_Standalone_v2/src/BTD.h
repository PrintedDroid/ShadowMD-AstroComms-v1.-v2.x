...
#define HCI_ACCEPT_DELAY_STATE                  20 // Non-blocking delay before accepting HID gamepad connections
...
private:
        ...
        uint32_t hci_accept_timer; // millis() timer for delayed HCI accept of HID gamepads
...
class BluetoothService {
public:
        BluetoothService(BTD *p) : pBtd(p), hci_handle(0xFFFF), l2cap_event_flag(0), pFuncOnInit(NULL) {
                if(pBtd)
        ...
        /**
         * Called by BTD when EV_DISCONNECT_COMPLETE fires.
         * Resets the service if the disconnected handle matches.
         * @param handle The HCI handle that was disconnected.
         */
        void checkDisconnect(uint16_t handle) {
                if(hci_handle == handle)
                        Reset();
        };

        /**
         * Called by BTD before accepting an incoming HCI connection.
         * Returns true if this service would accept a connection from this MAC.
         * Override in derived classes to implement MAC filtering.
         * @param bdaddr BD address of the incoming device (6 bytes).
         */
        virtual bool matchesMac(uint8_t* bdaddr) {
