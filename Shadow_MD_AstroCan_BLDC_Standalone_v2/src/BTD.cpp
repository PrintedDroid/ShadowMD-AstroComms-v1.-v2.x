...
uint8_t BTD::Init(uint8_t parent __attribute__((unused)), uint8_t port __attribute__((unused)), bool lowspeed) {
        ...
        if(VID == PS3_VID && (PID == PS3_PID || PID == PS3NAVIGATION_PID || PID == PS3MOVE_PID)) {
                ...
                // Read the controller's own BD address via HID report 0xF2
                // This populates disc_bdaddr so getLastConnectedBtMAC() can return it
                {
                        uint8_t buf[17];
                        pUsb->ctrlReq(bAddress, epInfo[BTD_CONTROL_PIPE].epAddr, bmREQ_HID_IN, HID_REQUEST_GET_REPORT, 0xF2, 0x03, 0x00, 17, 17, buf, NULL);
...
#ifdef DEBUG_USB_HOST
                Notify(PSTR(" (debug details above)"), 0x80);
#endif
        }
                ...
                switch(hcibuf[0]) { // Switch on event type
                        ...
                        case EV_CONNECT_COMPLETE:
                                ...
                                if(!hcibuf[2]) { // Check if connected OK
                                        ...
                                        hci_state = HCI_SCANNING_STATE; // Go back to scanning quickly on failure
#ifdef DEBUG_USB_HOST
                        ...
                        case EV_DISCONNECT_COMPLETE:
                                if(!hcibuf[2]) { // Check if disconnected OK
                                        ...
                                        // Notify services so they reset their connection state
                                        // Without this, PS3NavigationConnected stays true and
                                        // the service refuses to claim reconnections
                                        {
                                                uint16_t dh = (hcibuf[3] | ((hcibuf[4] & 0x0F) << 8));
                                                for(uint8_t i = 0; i < BTD_NUM_SERVICES; i++) {
                                                        if(btService[i])
                                                                btService[i]->checkDisconnect(dh);
                                                }
                                                Serial.println(F("\r\nDevice Disconnected"));
                                        }
                                }
...
#ifdef DEBUG_USB_HOST
                                        Notify(PSTR("\r\nRemote Name Failed: "), 0x80);
                                        D_PrintHex<uint8_t > (hcibuf[2], 0x80);
#endif
                                        remote_name[0] = '\0'; // Clear name on failure
                                }
                                hci_set_flag(HCI_FLAG_REMOTE_NAME_COMPLETE); // Always set flag so state machine can proceed
                                break;
        ...
        switch(hci_state) {
                ...
                case HCI_BDADDR_STATE:
                        if(hci_check_flag(HCI_FLAG_READ_BDADDR)) {
                                Serial.print(F("\r\nDongle Address: "));
                                for(int8_t i = 5; i > 0; i--) {
                                        if(my_bdaddr[i] < 0x10) Serial.print(F("0"));
                                        Serial.print(my_bdaddr[i], HEX);
                                        Serial.print(F(":"));
                                }
                                if(my_bdaddr[0] < 0x10) Serial.print(F("0"));
                                Serial.println(my_bdaddr[0], HEX);
#ifdef DEBUG_USB_HOST
                                Notify(PSTR(""), 0x80);
#endif
...
#endif
                                if(incomingHIDDevice) {
                                        // Skip remote name for gamepads - many dongles return 0x0C (Command Disallowed)
                                        // Instead, use a non-blocking delay to replace the implicit timing
                                        // that the Remote Name Request provided in the original library
                                        remote_name[0] = '\0';
                                        hci_accept_timer = millis();
                                        hci_state = HCI_ACCEPT_DELAY_STATE;
#ifdef DEBUG_USB_HOST
                                        Notify(PSTR("\r\nDelaying accept for HID gamepad..."), 0x80);
#endif
                                } else {
                                        hci_remote_name();
                                        hci_state = HCI_REMOTE_NAME_STATE;
                ...
                case HCI_ACCEPT_DELAY_STATE:
                        if((int32_t)((uint32_t)millis() - hci_accept_timer) > 300) { // 300ms delay before accepting
                                // Check if any service would accept this device's MAC
                                {
                                        bool macAccepted = false;
                                        for(uint8_t i = 0; i < BTD_NUM_SERVICES; i++) {
                                                if(btService[i] && btService[i]->matchesMac(disc_bdaddr)) {
                                                        macAccepted = true;
                                                        break;
                                                }
                                        }
                                        if(macAccepted) {
#ifdef DEBUG_USB_HOST
                                                Notify(PSTR("\r\nAccepting HID connection after delay"), 0x80);
#endif
                                                hci_accept_connection();
                                                hci_state = HCI_CONNECTED_STATE;
                                        } else {
                                                // Always print rejected MAC (security relevant)
                                                Serial.print(F("\r\nRejected: "));
                                                for(int8_t i = 5; i > 0; i--) {
                                                        if(disc_bdaddr[i] < 0x10) Serial.print(F("0"));
                                                        Serial.print(disc_bdaddr[i], HEX);
                                                        Serial.print(F(":"));
                                                }
                                                if(disc_bdaddr[0] < 0x10) Serial.print(F("0"));
                                                Serial.println(disc_bdaddr[0], HEX);
                ...
                case HCI_CONNECTED_STATE:
                        if(hci_check_flag(HCI_FLAG_CONNECT_COMPLETE)) {
                                // Always print connected device MAC (important status)
                                Serial.print(F("\r\nConnected: "));
                                for(int8_t i = 5; i > 0; i--) {
                                        if(disc_bdaddr[i] < 0x10) Serial.print(F("0"));
                                        Serial.print(disc_bdaddr[i], HEX);
                                ...
                                hci_event_flag = 0;
                                hci_counter = 0; // Reset counter for L2CAP timeout in HCI_DONE_STATE
                ...
                case HCI_DONE_STATE:
                        ...
                        if(hci_counter > 1000) { // Wait until we have looped 1000 times to make sure that the L2CAP connection has been started
                                hci_counter = 0;
                                if(!l2capConnectionClaimed) {
                                        // L2CAP was never claimed - controller connected at HCI but L2CAP didn't start
                                        // Disconnect so controller will retry cleanly
#ifdef DEBUG_USB_HOST
                                        Notify(PSTR("\r\nL2CAP timeout - disconnecting for retry"), 0x80);
#endif
                                        hci_disconnect(hci_handle);
                                        hci_state = HCI_DISCONNECT_STATE;
                                } else {
...
void BTD::hci_reject_connection() {
        hcibuf[0] = 0x0A; // HCI OCF = 0x0A (Reject_Connection_Request)
        hcibuf[1] = 0x01 << 2; // HCI OGF = 1
        hcibuf[2] = 0x07; // parameter length 7
        hcibuf[3] = disc_bdaddr[0]; // 6 octet bdaddr
        hcibuf[4] = disc_bdaddr[1];
        ...
        hcibuf[7] = disc_bdaddr[4];
        hcibuf[8] = disc_bdaddr[5];
        hcibuf[9] = 0x0F; // Reason: Unacceptable BD_ADDR
