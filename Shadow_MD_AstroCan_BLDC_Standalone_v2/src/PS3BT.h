...
public:
        ...
        /**
         * Set a MAC address filter. When set, this service will only claim
         * incoming connections from devices matching one of the filter MACs.
         * MAC bytes are in LSB-first order (same as disc_bdaddr).
         * @param primary  Primary MAC to accept (6 bytes, LSB-first)
         * @param backup   Optional backup MAC to also accept (6 bytes, LSB-first), NULL if none
         */
        void setMacFilter(const uint8_t* primary, const uint8_t* backup = NULL);

        /** Clear the MAC filter - accept connections from any device. */
        void clearMacFilter();

        /**
         * Check if this service would accept a connection from the given MAC.
         * Used by BTD to reject unknown devices at HCI level before accepting.
         * @param bdaddr BD address to check (6 bytes).
         * @return true if MAC matches filter (or no filter is set).
         */
        bool matchesMac(uint8_t* bdaddr);
        /**@}*/
...
private:
        ...
        /* MAC address filter for connection claiming */
        uint8_t mac_filter_primary[6];
        uint8_t mac_filter_backup[6];
        bool has_mac_filter;
        bool has_backup_filter;
