...
{
        has_mac_filter = false;
        has_backup_filter = false;
...
void PS3BT::Reset() {
        ...
        hci_handle = 0xFFFF; // Invalid handle - prevents accidental ACL data matching
        l2cap_event_flag = 0; // Reset flags
        l2cap_state = L2CAP_WAIT;
        timerHID = 0; // Prevent garbage value causing ~24 day delay in HID_Command
...
void PS3BT::setMacFilter(const uint8_t* primary, const uint8_t* backup) {
        for(uint8_t i = 0; i < 6; i++)
                mac_filter_primary[i] = primary[i];
        has_mac_filter = true;
        if(backup) {
                for(uint8_t i = 0; i < 6; i++)
                        mac_filter_backup[i] = backup[i];
                has_backup_filter = true;
        } else {
                has_backup_filter = false;
        }
        ...
        for(int8_t i = 5; i > 0; i--) {
                D_PrintHex<uint8_t > (mac_filter_primary[i], 0x80);
                Notify(PSTR(":"), 0x80);
        }
        D_PrintHex<uint8_t > (mac_filter_primary[0], 0x80);
#endif
}

void PS3BT::clearMacFilter() {
        has_mac_filter = false;
        has_backup_filter = false;
#ifdef DEBUG_USB_HOST
        Notify(PSTR("\r\nMAC filter cleared"), 0x80);
#endif
}

bool PS3BT::matchesMac(uint8_t* bdaddr) {
        if(!has_mac_filter)
                return true; // No filter = accept all (pairing mode)
        bool match = true;
        for(uint8_t i = 0; i < 6; i++) {
                if(bdaddr[i] != mac_filter_primary[i]) { match = false; break; }
        }
        if(match) return true;
        if(has_backup_filter) {
                match = true;
                for(uint8_t i = 0; i < 6; i++) {
                        if(bdaddr[i] != mac_filter_backup[i]) { match = false; break; }
                }
                if(match) return true;
        }
        return false;
        ...
        if(!pBtd->l2capConnectionClaimed && !PS3Connected && !PS3MoveConnected && !PS3NavigationConnected && !activeConnection && !pBtd->connectToWii && !pBtd->incomingWii && !pBtd->pairWithWii) {
                if(ACLData[8] == L2CAP_CMD_CONNECTION_REQUEST) {
                        if((ACLData[12] | (ACLData[13] << 8)) == HID_CTRL_PSM) {
                                // Check MAC filter before claiming connection
                                if(has_mac_filter) {
                                        bool mac_match = true;
                                        for(uint8_t i = 0; i < 6; i++) {
                                                if(pBtd->disc_bdaddr[i] != mac_filter_primary[i]) {
                                                        mac_match = false;
                                                        break;
                                                }
                                        }
                                        if(!mac_match && has_backup_filter) {
                                                mac_match = true;
                                                for(uint8_t i = 0; i < 6; i++) {
                                                        if(pBtd->disc_bdaddr[i] != mac_filter_backup[i]) {
                                                                mac_match = false;
                                                                break;
                                                        }
                                                }
                                        ...
                                        if(!mac_match) {
#ifdef DEBUG_USB_HOST
                                                Notify(PSTR("\r\nMAC filter: connection rejected"), 0x80);
#endif
                                                return; // Don't claim this connection
                                        }
                                }
                                ...
                                l2cap_state = L2CAP_WAIT;
                                // Remote name is skipped for HID gamepads (dongle returns 0x0C).
                                // If MAC filter matched, we KNOW it's a Navigation Controller.
                                // Without MAC filter (pairing mode), also treat as Navigation since
                                // this system exclusively uses PS3 Move Navigation Controllers.
                                remote_name_first = 'N'; // Navigation Controller
#ifdef DEBUG_USB_HOST
...
#ifdef DEBUG_USB_HOST
                                Notify(PSTR("\r\n[PS3BT] L2CAP done -> "), 0x80);
                                Notify(l2cap_state == TURN_ON_LED ? PSTR("TURN_ON_LED") : PSTR("PS3_ENABLE_SIXAXIS"), 0x80);
#endif
                        }
                        break;
        ...
        switch(l2cap_state) {
                case PS3_ENABLE_SIXAXIS:
                        if((int32_t)((uint32_t)millis() - timer) > 1000) { // loop 1 second before sending the command
                                ...
                                // enable_sixaxis is needed for BOTH DualShock 3 AND Navigation Controller
                                // (enables BT data output, not just motion sensor)
#ifdef DEBUG_USB_HOST
                                Notify(PSTR("\r\n[PS3BT] Sending enable_sixaxis()"), 0x80);
#endif
                                enable_sixaxis();
#ifdef DEBUG_USB_HOST
                                Notify(PSTR("\r\n[PS3BT] enable_sixaxis() done"), 0x80);
#endif
                                l2cap_state = TURN_ON_LED;
                                timer = (uint32_t)millis();
                        }
                ...
                case TURN_ON_LED:
                        if((int32_t)((uint32_t)millis() - timer) > 1000) { // loop 1 second before sending the command
#ifdef DEBUG_USB_HOST
                                Notify(PSTR("\r\n[PS3BT] TURN_ON_LED, name_first="), 0x80);
                                D_PrintHex<uint8_t>(remote_name_first, 0x80);
#endif
                                ...
                                } else { // Unknown controller type - reject connection
                                        Serial.println(F("\r\nUnknown Controller - disconnecting!"));
                                        // Disconnect unknown controller - safety measure
                                        pBtd->hci_disconnect(hci_handle);
                                        Reset();
                                        break; // Don't call onInit
                                }
...
#ifdef DEBUG_USB_HOST
        int32_t diff = (int32_t)((uint32_t)millis() - timerHID);
        Notify(PSTR("\r\n[HID_Cmd] diff="), 0x80);
        PrintHex<int32_t>(diff, 0x80);
        Notify(PSTR(" h="), 0x80);
        D_PrintHex<uint16_t>(hci_handle, 0x80);
        Notify(PSTR(" scid="), 0x80);
        D_PrintHex<uint8_t>(control_scid[1], 0x80);
        D_PrintHex<uint8_t>(control_scid[0], 0x80);
        Notify(PSTR(" n="), 0x80);
